...
class PrefixScope(val bot: Bot, val prefix: String) : Scope<GeneralCommandExecutor, GeneralCommandHandler>{
    ...
    class GroupCommandScope(val bot: Bot, val prefixScope: PrefixScope, val scope: CommandWorkingScope = CommandWorkingScope.EVERYONE, var groupFilter : (Group) -> Boolean = { true }) : Scope<GroupCommandExecutor, GroupCommandHandler> {
        override fun String.invoke(handler: suspend GroupCommandExecutor.() -> Unit)= GroupCommandHandler(bot, "${prefixScope.prefix}$this", scope , handler).apply { this.groupFilter = this@GroupCommandScope.groupFilter }
        ...
        @CommandDSL fun operators(b : GroupCommandScope.() -> Unit) = GroupCommandScope(bot, prefixScope, CommandWorkingScope.ADMIN_AND_OWNER, groupFilter).apply(b)
